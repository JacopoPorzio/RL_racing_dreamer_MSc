...
class Lidar(BulletSensor[NDArray[(Any,), np.float]]):
    ...
    def _setup_raycast(self, scan_range: float, rays: int):
        ...
        # otherwise it will ignore walls that are closer than  ̀mindist` from the sensor
        start = self._config.min_range  # 0.0
        end = scan_range
    ...
    def observe(self) -> NDArray[(Any,), np.float]:
        ...
        # The bias term in ranges is useful, in order to actually "unbias" the fact that the LiDAR rays
        # don't start from 0.0, but from min_range
