...
class RandomPositioningWithinBallStrategy(RandomPositioningStrategy):
    ...
    def __init__(self, progress_map: GridMap, obstacle_map: GridMap, drivable_map: np.ndarray, centerline_map: np.array,
                 min_distance_to_obstacle: float = 0.5, progress_center: float = 0.0, progress_radius: float = 0.01):
        self._progress = copy.copy(progress_map)
        self._obstacles = copy.copy(obstacle_map)
        self._occupancy = copy.copy(drivable_map)
        self._centerline = copy.copy(centerline_map)
        self._centerline = (self._centerline/255).astype(int)
        self._obstacle_margin = min_distance_to_obstacle
    ...
    def get_pose(self, agent_index: int) -> Pose:
        ...
        #                                              (self._occupancy * self._obstacles.map) >= self._obstacle_margin))  # OLD

        # progress_map_reduced = self._progress.map*((self._occupancy * self._obstacles.map) >= self._obstacle_margin)  # OLD
        progress_map_reduced = self._progress.map * self._centerline * self._occupancy
        center_corridor = np.argwhere(np.logical_and(progress_interval, progress_map_reduced))
        progress_reduced = self._progress
        progress_reduced._map = progress_map_reduced
        x, y, angle = self._random_position(progress_reduced, center_corridor, 0.0035)
        # x, y, angle = self._random_position(self._progress, center_corridor, 0.0035)
        return (x, y, 0.05), (0, 0, angle)
