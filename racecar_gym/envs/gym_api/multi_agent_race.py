...
class MultiAgentRaceEnv(gymnasium.Env):
    ...
    def reset(self, *, seed: Optional[int] = None, options: Dict[str, Any] = None) -> Tuple[ObsType, Dict[str, Any]]:
        ...
        # starting_state = observations.copy()  ###############
        starting_state = {}
        for agent in self._scenario.agents.values():
            starting_state[agent.id] = {**observations[agent.id], **state[agent.id]}
            # starting_state[agent.id].update(state[agent.id].copy())
            self._scenario.agents[agent.id].task.reset(starting_state)
