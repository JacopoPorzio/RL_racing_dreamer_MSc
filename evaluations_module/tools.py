...
def video_summary(name, video, step=None, fps=20):
    ...
    try:
        ...
        summary.value.add(tag=name + '/gif', image=image)
        tf.summary.experimental.write_raw_pb(summary.SerializeToString(), step)
        print(f'Imagination summary GIF of episode number {step} was saved.')
    except:
...
def simulate(agent, envs, steps=0, episodes=0, state=None):
  ...
  if state is None:
    step, episode = 0, 0
    done = True  # np.ones(len(envs), np.bool)
  ...
  while (steps and step < steps) or (episodes and episode < episodes):
    ...
    if done:
      obs, info = envs[0].reset(reset_options)
      cum_reward = 0.0
    ...
    # if len(obs) == 2:
    #     obs_to_pass = {k: np.stack([v]) for k, v in obs[0][0].items()}
    # if len(obs) == 1:
    #     obs_to_pass = {k: np.stack([v]) for k, v in obs[0].items()}
    obs_to_pass = {k: np.stack([v]) for k, v in obs.items()}
    action, agent_state = agent(obs_to_pass, done, agent_state)
    action = action[0]
    # print(action)
    # action = {'steering': action[0], 'motor': action[1]}
