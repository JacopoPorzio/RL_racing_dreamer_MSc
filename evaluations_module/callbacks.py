...
import evaluations_module.tools as tools
import numpy as np
...
def save_eval_videos(videos, video_dir, track):
    # this is used in the evaluation
    video_dir.mkdir(parents=True, exist_ok=True)
    episodes = tools.count_videos(video_dir)
    for filename, video in videos.items():
        writer = imageio.get_writer(f'{video_dir}/{filename}_{episodes + 1}_{track}.mp4', fps=25)  # fps=100
        for image in video:
            writer.append_data(image)
        writer.close()
...
def save_episodes(directory, episodes):
    episode_number = tools.count_videos(directory / 'videos')
    directory = directory / 'episodes'
    ...
    for episode in episodes:
        ...
        filename = directory / f'Episode_{episode_number}.npz'
...
def summarize_eval_episode(episodes, outdir, writer, prefix):
...
def save_trajectory(episodes, outdir, track):
    trajectory_dir = outdir / 'trajectories'
    trajectory_dir.mkdir(parents=True, exist_ok=True)
    episode = episodes  # [0]
    # assert 'pose' in episode and 'velocity' in episode
    assert 'velocity' in episode
    # As of now, we have no pose: we can create a specific .yml, in order to add this as a sensor
    # TODO: GET IT FROM STATE IN SOME WRAPPER
    episodes = tools.count_videos(outdir / 'videos')
    # positions = episode['pose'][:, :2]
    velocities = episode['velocity'][:, 0]
    commands = episode['action']
    times = episode['time']
    filename = trajectory_dir / f"trajectory_{episodes}_{track}"
    # np.savez(filename, position=positions, velocity=velocities, actions=commands)
    np.savez(filename, velocity=velocities, actions=commands, time=times)
