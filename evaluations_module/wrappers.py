...
from racecar_gym.envs import MultiAgentScenario
...
class Render:
    ...
    def step(self, action):
        obss, reward, done, truncated, info = self._env.step(action)
    ...
    def reset(self, reset_options):
        obs, info = self._env.reset(reset_options)
        for k in self._videos.keys():
            # mode, agent = k.split('-')
            frame = self._env.render()
...
class Collect:
    ...
    def step(self, action):
        ...
        transition['progress'] = info['progress']
        ...
        if done:
            ...
            episode = {k: self._convert_action(v) if k == 'action' else self._convert(v) for k, v in episode.items()}
    ...
    def _convert_action(self, value):
        tmp = {'steering': value[0][0], 'motor': value[0][1]}
        value[0] = tmp
        return value
